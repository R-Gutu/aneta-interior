    ...
    return (
        <div className="pt-20 sm:pt-32 lg:pt-40">
            ...
            <div className="flex flex-col gap-10 px-4 sm:px-8 md:px-12 lg:px-16 xl:px-20">
                {projects
                    .filter((p) => section in p)
                    .map((p, i) => {
                        if (!p) return null;

                        let images = p[section as SectionKey]?.map((o) => o.image) ?? [];

                        // Специально для ванной: меняем первую и вторую фотку
                        if (section === "bathroom" && images.length >= 2) {
