...
export default function Slider({ images, key: keyProp, className }: { images: string[], key?: number, className?: string }) {
  ...
  useEffect(() => {
    const preloadNext = (startIndex: number, count: number) => {
      for (let i = 0; i < count; i++) {
        ...
        if (!loadedImages.has(index)) {
          ...
          img.onerror = () => setLoadedImages(prev => new Set([...prev, index]));
    ...
    if (!loadedImages.has(prevIndex)) {
      ...
      img.onerror = () => setLoadedImages(prev => new Set([...prev, prevIndex]));
  ...
  const goToPrevious = () => goToSlide(currentIndex === 0 ? images.length - 1 : currentIndex - 1);

  return (
    <div key={keyProp || 0} className={`relative flex flex-col items-center w-full mx-auto overflow-hidden ${className || ''}`}>
      {/* Image Container */}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <Image
              ...
              className="w-full h-full object-cover rounded-tl-[100px] rounded-br-[100px] max-[800px]:rounded-tl-[20px] max-[800px]:rounded-br-[20px]" 
